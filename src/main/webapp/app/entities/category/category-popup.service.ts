...
export class CategoryPopupService {
        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.categoryService.find(id).subscribe((data) => {
                    this.ngbModalRef = this.categoryModalRef(component, data);
                    resolve(this.ngbModalRef);
                });
            } else {
    ...
    load(component: Component): Promise<NgbModalRef> {
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            // setTimeout used as a workaround for getting ExpressionChangedAfterItHasBeenCheckedError
            setTimeout(() => {
                this.ngbModalRef = this.categoryLoadModalRef(component);
                resolve(this.ngbModalRef);
            }, 0);
        });
    }

    categoryLoadModalRef(component: Component): NgbModalRef {
        const modalRef = this.modalService.open(component, { size: 'lg', backdrop: 'static'});
        modalRef.result.then((result) => {
            this.router.navigate([{ outlets: { popup: null }}], { replaceUrl: true });
            this.ngbModalRef = null;
