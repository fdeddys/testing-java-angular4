...
import id.cls.service.ProductService;
...
public class ProductResource {
    ...
    /**
     * POST  /products/execute{id}/{param} : Execute Bussiness Process product.
     *
     * @param productDTO the productDTO to Execute
     * @param id the id process to Execute
     * @param param the paramater process to Execute
     * @return the ResponseEntity with status Accepted and with body the  productDTO
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/products/execute/{id}/{param}")
    @Timed
    public ResponseEntity<ProductDTO> executedProduct(@PathVariable Integer id, @PathVariable String param, @RequestBody ProductDTO productDTO) throws URISyntaxException {
        log.debug("REST request to process Product : {}", productDTO);
        ProductDTO result = productService.processExecuteData(id, param, productDTO);
        return new ResponseEntity<ProductDTO>(result, null, HttpStatus.OK);
    }

    /**
     * POST  /products/execute-list{id}/{param} : Execute Bussiness Process product.
     *
     * @param productDTO the productDTO to Execute
     * @param id the id process to Execute
     * @param param the paramater process to Execute
     * @return the ResponseEntities with status Accepted and with body the  productDTO
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/products/execute-list/{id}/{param}")
    @Timed
    public ResponseEntity<Set<ProductDTO>> executedListProduct(@PathVariable Integer id, @PathVariable String param, @RequestBody Set<ProductDTO> productDTO) throws URISyntaxException {
        log.debug("REST request to process List Product");
        Set<ProductDTO> result = productService.processExecuteListData(id, param, productDTO);
        return new ResponseEntity<Set<ProductDTO>>(result, null, HttpStatus.OK);
    }
