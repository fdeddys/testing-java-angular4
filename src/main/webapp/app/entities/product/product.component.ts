...
export class ProductComponent implements OnInit, OnDestroy {
    ...
    executeProcess(data) {
        this.productService.executeProcess(0, null, data).subscribe(
    ...
    updateRowData(event) {
        if (event.data.id !== undefined) {
            this.productService.update(event.data)
                .subscribe((res: Product) =>
                    this.onRowDataSaveSuccess(res), (res: Response) => this.onRowDataSaveError(res));
        } else {
            this.productService.create(event.data)
                .subscribe((res: Product) =>
                    this.onRowDataSaveSuccess(res), (res: Response) => this.onRowDataSaveError(res));
        }
    }

    private onRowDataSaveSuccess(result: Product) {
        this.toasterService.showToaster('info', 'Product Saved', 'Data saved..');
    }
